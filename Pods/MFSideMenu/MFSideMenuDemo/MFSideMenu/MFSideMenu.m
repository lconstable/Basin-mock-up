...
- (UIBarButtonItem *)menuBarButtonItem {
    return [[UIBarButtonItem alloc]
            initWithImage:[UIImage imageNamed:@"sidebarMenuIcon.png"] style:UIBarButtonItemStyleBordered
            target:self
    ...
    if([self menuButtonEnabled]) {
        if(self.menuSide == MFSideMenuLocationRight && !navigationItem.rightBarButtonItem) {
            navigationItem.rightBarButtonItem = [self menuBarButtonItem];
                  ...
                  (self.menuState == MFSideMenuStateVisible || self.navigationController.viewControllers.count == 1 || !navigationItem.leftBarButtonItem)) {
#warning added last clause--test for problems
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        // LEC: I decided to override UITableViewCell swipes
        if ([touch.view isKindOfClass:[UITableViewCell class]] ||
            [touch.view.superview isKindOfClass:[UITableViewCell class]]) return YES;
